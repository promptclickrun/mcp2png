...
export async function copyPngToClipboard(blob: Blob, options: ClipboardOptions = {}): Promise<void> {
  ...
  if (normalized.requireUserActivation && !hasUserActivation()) {
    ...
    throw new Mcp2PngError(
      'MISSING_USER_ACTIVATION',
      'Clipboard image writes must be triggered by a trusted user gesture such as a click.',
    );
  }
...
export async function copyElementAsPng(target: ElementLike, options: CopyElementOptions = {}): Promise<CopyResult> {
...
export function downloadPng(blob: Blob, filename = 'mcp-app.png'): string {
